#!/usr/bin/env python3
"""
SeniorTalk 测试集评估 - 修复版（使用用户主目录）
所有输出和缓存都保存到 /home/saisai 下，避免 /data 只读问题
"""
...
print(f"{'='*80}")
print(f"Hugging Face 缓存目录: {HF_CACHE_DIR}")
print(f"{'='*80}\n")
...
print(f"{'='*80}")
print(f"SeniorTalk Evaluation - Fixed Version")
print(f"{'='*80}")
    ...
    for parquet_file in sorted(parquet_files):
        ...
        try:
            ...
            for idx, row in df.iterrows():
                ...
                try:
                    # 从 Parquet 提取音频和文本
                    # 根据实际的列名调整（可能是 'audio', 'text' 或 'sentence'）
                    if hasattr(row, 'audio'):
                        # 方式1: 如果 audio 是字典，包含 bytes
                        if isinstance(row.audio, dict) and 'bytes' in row.audio:
                            audio_bytes = row.audio['bytes']
                        # 方式2: 如果 audio 直接是 bytes
